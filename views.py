...
def prdict_heart_disease(list_data):
    ...
    # Convert 'm' and 'f' back to 0 and 1 for the model prediction
    list_data[1] = 0 if list_data[1] == 'm' else 1
    
    pred = nn_model.predict([list_data])

    accuracy = nn_model.score(X_test, y_test) * 100
    accuracy_rounded = round(accuracy, 2)  # Round the accuracy to 2 decimal places
    print("Neural Network Accuracy: {:.2f}%".format(accuracy_rounded))
    print("Predicted Value is : ", format(pred))

    dataframe = str(df.head())
    return accuracy_rounded, pred
    ...
    if request.method == "POST":
        ...
        for key, value in value_dict.items():
            ...
            if key == "sex" and value[0].lower() in ['male', 'm']:
                list_data.append('m')  # Use 'm' for male
                continue
            elif key == "sex" and value[0].lower() in ['female', 'f']:
                list_data.append('f')  # Use 'f' for female
                continue
